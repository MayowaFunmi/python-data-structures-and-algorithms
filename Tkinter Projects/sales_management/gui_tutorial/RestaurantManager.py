...
from tkinter import *
import random #for reference mumber
import time

#app name
root = Tk()
#root.geometry("1600x800+0+0") #app size
root.title("Restaurant Management System")

text_Input = StringVar()
operator = ""
#create 3 frames
Tops = Frame(root, width = 1600, height = 50, bg = "powder blue", relief = SUNKEN).pack(side = TOP)

f1 = Frame(root, width = 800, height = 700, relief = SUNKEN).pack(side = LEFT)

f2 = Frame(root, width = 300, height = 700, relief = SUNKEN).pack(side = RIGHT)
...
#The calculator
#display calculator screen with line 12
...
def Ref():
    ...
    #get entered quantities of each item
    CoF = float(Fries.get())
    ...
    #add together as CostofMeal
    CostofMeal ="£", str("%.2f" % (CostofFries + CostofDrinks + CostofFilet + CostofBurger + CostChicken_Burger + CostCheese_Burger))
    
    #calc tax on CostofMeal
    PayTax = ((CostofFries + CostofDrinks + CostofFilet + CostofBurger + CostChicken_Burger + CostCheese_Burger) * 0,2)
...
multiply = Button(f2, padx = 16, pady = 16, bd = 8, fg = "black", font = ("arial", 20, "bold"), text ="X", bg = "powder blue", command = lambda: btnClick("X")).grid(row = 4, column = 3)
#----------------------------
...
#---------Restaurant Info 1-------------------------------------
rand = StringVar() #declare variable
Fries = StringVar()
Burger = StringVar()
Filet = StringVar()
SubTotal = StringVar()
